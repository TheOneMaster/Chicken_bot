...
const command: SlashCommand = {
  ...
  async execute(interaction) {
    ...
    const accounts = await AppDataSource.getRepository(Account).findBy({
      region: region,
    });
        ...
        if (account.puuid) {
          ...
          return new EmbedBuilder()
            ...
            .addFields([
              ...
              {
                name: "Rank",
                value: rankDetails.currenttierpatched,
                inline: true,
              },
            ])
            .setFooter({
              iconURL: playerCard ? rankIcon : undefined,
              text: null!,
            });
        }

        return new EmbedBuilder().setColor(0x00ffff).addFields([
          { name: "Username", value: account.accountName, inline: true },
          { name: "Password", value: account.password, inline: true },
        ]);
      })
